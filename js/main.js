...
class Fire {
    ...
    render() {
        ...
        this.particles.forEach((particle) => particle.render(this.currentFrame, this.ctx));
...
class Particle {
    ...
    render(currentFrame, ctx) {
        const currTime = new Date().getTime();
        const frame = ((currTime - this.init_time) / 1000) * 60;
        ctx.fillStyle = `rgb(250,${250 - (frame / 60) * 250},0,${1 - frame / 60})`;
        ctx.beginPath();
        const randomNumber = Math.random();
        ctx.arc(this.init_x + Math.sin((frame / 4) * randomNumber), this.init_y - (frame * this.init_size) / 7, this.init_size / (frame / 20 + 1), 0, 2 * Math.PI);
        ctx.fill();
    }
}
